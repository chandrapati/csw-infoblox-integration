...
MD="$DIR/CSW-Infoblox-Integration-Guide.md"
HTML="$DIR/CSW-Infoblox-Integration-Guide.html"
PDF="$DIR/CSW-Infoblox-Integration-Guide.pdf"
...
echo "$CSS" | pandoc "$MD" \
  --standalone \
  --embed-resources \
  --metadata title="Cisco Secure Workload — Infoblox Integration Guide" \
  --css /dev/stdin \
  -o "$HTML"
...
# Inject a responsive YouTube iframe right after the walkthrough link
python3 - "$HTML" << 'PY'
import sys, re
path = sys.argv[1]
html = open(path).read()
...
iframe_block = f"""
<div class="yt-wrap">
  <iframe src="{EMBED_URL}" title="Cisco Secure Workload - Infoblox Integration"
    allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"
    referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
</div>
<div class="yt-pdf-box">
  <div class="yt-title">&#9654; Video: Cisco Secure Workload &amp; Infoblox Integration</div>
  <div class="yt-url">{YT_URL}</div>
  <div class="yt-meta">Cisco Secure Workload official channel &middot; Open in browser to watch</div>
</div>"""
pattern = r'(<a\s+href="https://www\.youtube\.com/watch\?v=gdhMWviAZig"[^>]*>.*?</a>)'
